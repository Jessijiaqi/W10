...
class Player:
    ...
    # def record_winner(text, filename, directory, first_row, first_col):
    #     file_path = os.path.join(directory, filename)
    #     with open(file_path, 'a') as f:
    #         f.write(text + '\n')
    #         f.write(f"First Input: Row {first_row}, Column {first_col}\n\n")
...
class Game:
    ...
    def run(self):
        ...
        while winner is None:
            ...
            if first_row is None and first_col is None:
                first_row, first_col = row, col  # Store the first input
        ...
        if winner == "draw":
            ...
            game_result = f"X {'wins' if winner =='X' else 'loses'}"
